...
class DuplicateDetector:
    ...
    def _assign_group_ids(self, df: DataFrame, key_columns: List[str]) -> DataFrame:
        ...
        # Use dense_rank to assign sequential group IDs
        window_spec = Window.partitionBy(*key_columns).orderBy(*key_columns)
        
        # Assign a unique ID to each group of duplicates
        # We'll use row_number over the entire dataset ordered by key columns
        # to create a unique group ID for each unique combination
        
        # First, create a unique identifier for each key combination
        # by using dense_rank over all rows ordered by key columns
        global_window = Window.orderBy(*key_columns)
        
        df_with_group_id = df.withColumn(
            "duplicate_group_id",
            row_number().over(global_window)
        )
        
        # However, we need all rows with the same key to have the same group_id
        # So we'll use a different approach: get the minimum row number for each group

    ...
    def convert_types_with_detection(self, df: pd.DataFrame, table_name: str, 
        ...
        for col in df.columns:
            ...
            if df[col].dtype == 'object':
                ...
                if numeric_count / total_sample > 0.95:
                    ...
                    # Convertir a string PRESERVANDO los NULL
                    df[col] = df[col].apply(lambda x: str(x) if pd.notna(x) else None)
                    
                    # Limpiar valores especiales que pudieran quedar
        ...
        for col in df.columns:
            if df[col].dtype == 'object':
                ...
                # Convertir a string PRESERVANDO NULL
                df[col] = df[col].apply(lambda x: str(x) if pd.notna(x) else None)
                df[col] = df[col].replace('None', None)
                df[col] = df[col].replace('nan', None)
                column_types[col] = 'string'
    ...
    def apply_column_types(self, df: pd.DataFrame, column_types: Dict) -> pd.DataFrame:
        ...
        for col, col_type in column_types.items():
            ...
            if col_type == 'numeric':
                ...
                # No hacer nada, mantener los NULL como están
                pass
